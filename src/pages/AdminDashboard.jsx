...
// StatCard for displaying statistics (reused and slightly enhanced)
...
// Card component for sections like Moderation, Settings, etc.
...
const MintBurnCacao = () => {
  ...
  const findUser = async () => {
    ...
    // Replace with actual Supabase query to find user by email or name
    // Example: const { data, error } = await supabase.from('users').select('*').or(`email.ilike.%${searchQuery}%,name.ilike.%${searchQuery}%`).limit(1);
  ...
  const handleMintBurn = async (action) => {
    ...
    // Example: await supabase.rpc('update_user_cacao_beans', { user_id: userStats.id, amount: action === 'mint' ? numericAmount : -numericAmount, reason: reason });
        ...
        <div className="bg-orange-50 rounded-xl p-5 border border-orange-100">
          ...
          <div className="flex flex-col sm:flex-row gap-3 mb-3">
            <input
              ...
              className="flex-grow p-3 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 space-grotesk"
            />
            <button
              onClick={findUser}
              className="px-5 py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-lg shadow transition-colors duration-200 space-grotesk"
            >
              Tìm kiếm
            </button>
          </div>
          {userStats && (
            <div className="mt-4 p-4 bg-orange-100 rounded-lg border border-orange-200 space-grotesk">
              ...
              <p className="font-bold text-lg text-orange-900 mt-2">
                Số Hạt Cacao Hiện tại: {userStats.cacaoBeans} 🌰
              </p>
            </div>
          )}
          ...
          <div className="bg-orange-50 rounded-xl p-5 border border-orange-100">
            ...
            <div className="flex flex-col md:flex-row gap-3 mb-3">
              <input
                ...
                placeholder="Số lượng hạt"
                className="p-3 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 space-grotesk w-full md:w-1/2"
              />
              <textarea
                value={reason}
                onChange={(e) => setReason(e.target.value)}
                placeholder="Lý do (bắt buộc)"
                className="p-3 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 space-grotesk w-full h-24 md:h-auto resize-none"
                rows="2"
              ></textarea>
            </div>
            <div className="flex gap-3">
              <button
                onClick={() => handleMintBurn('mint')}
                className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg shadow transition-colors duration-200 space-grotesk font-bold flex-1"
              >
                Bơm thêm hạt
              </button>
              <button
                onClick={() => handleMintBurn('burn')}
                className="px-4 py-2 bg-red-700 hover:bg-red-800 text-white rounded-lg shadow transition-colors duration-200 space-grotesk font-bold flex-1"
              >
                Tịch thu hạt
              </button>
            </div>
...
const CacaoLoungeModeration = () => {
  ...
  const handleAction = (postId, action) => {
    ...
    if (action === 'delete') {
      // Simulate banning user for 3 days
            ...
            <div key={post.id} className="bg-orange-50 rounded-xl p-4 shadow border border-orange-100 space-grotesk">
              ...
              <div className="flex space-x-3 mt-3">
                <button
                  ...
                  className="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-lg shadow transition-colors duration-200"
                >
                  Giữ lại bài viết
                </button>
                <button
                  onClick={() => handleAction(post.id, 'delete')}
                  className="px-4 py-2 bg-red-800 hover:bg-red-900 text-white rounded-lg shadow transition-colors duration-200 font-bold"
                >
                  Xóa & Cấm 3 ngày
                </button>
              </div>
...
const MetricsLab = () => {
  ...
  const [chartData, setChartData] = useState({
    hourlyTraffic: [150, 200, 250, 300, 400, 500, 600, 700, 650, 600, 550, 500, 450, 400, 350, 300, 350, 400, 450, 500, 550, 600, 500, 400], // 24 hours
    popularSubjects: [
      { name: 'Toán học', value: 70, color: '#D97706' }, // Orange-600
      { name: 'Vật lý', value: 55, color: '#B45309' }, // Orange-700
      { name: 'Hóa học', value: 48, color: '#92400E' }, // Orange-800
      { name: 'Tiếng Anh', value: 62, color: '#7C3A0F' }, // Orange-900
      { name: 'Lịch sử', value: 30, color: '#A16207' }, // Amber-700
    ],
    activeClasses: [
      { name: '10A1', value: 85, color: '#EA580C' }, // Orange-500
      { name: '11B3', value: 92, color: '#D97706' }, // Orange-600
      { name: '12C2', value: 78, color: '#B45309' }, // Orange-700
      { name: '9C1', value: 65, color: '#92400E' }, // Orange-800
    ],
  });

  // Simulate fetching data
  useEffect(() => {
    // In a real app, fetch data here and update chartData state
    // For now, we use static dummy data
  }, []);
    ...
    <AdminSectionCard title="Phòng Thí Nghiệm Metrics" icon="📈">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-orange-50 rounded-xl p-6 shadow border border-orange-100">
          ...
          <div className="space-y-3">
            {chartData.popularSubjects.map((subject, i) => (
      ...
      <div className="mt-6 bg-orange-50 rounded-xl p-6 shadow border border-orange-100">
        ...
        <div className="flex justify-around items-end h-40 space-grotesk">
          {chartData.activeClasses.map((cls, i) => (
            <div key={cls.name} className="flex flex-col items-center w-1/5">
              <div
                ...
                style={{
                  height: `${(cls.value / maxClassValue) * 100}%`,
...
const SystemSettings = () => {
  ...
  const handleSettingChange = (key, value) => {
    // Deep update for nested objects like exchangeRate or enrollmentPeriod
    setSettings(prev => {
    ...
    <AdminSectionCard title="Cấu hình Mùa Giải Cacao" icon="⚙️">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between space-grotesk">
              ...
              <div className="flex items-center space-x-2">
                <input
                  type="number"
                  value={settings.exchangeRate.homeworkToBeans}
                  onChange={(e) => handleSettingChange('exchangeRate.homeworkToBeans', parseInt(e.target.value, 10) || 0)}
                  className="p-2 w-20 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-center space-grotesk"
                />
                <span> Hạt/Bài</span>
              </div>
            </div>
            <div className="flex items-center justify-between space-grotesk">
              ...
              <div className="flex items-center space-x-2">
                <input
                  type="number"
                  value={settings.exchangeRate.quizToBeans}
                  onChange={(e) => handleSettingChange('exchangeRate.quizToBeans', parseInt(e.target.value, 10) || 0)}
                  className="p-2 w-20 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-center space-grotesk"
                />
                <span> Hạt/Quiz</span>
              </div>
        ...
        <div className="bg-orange-50 rounded-xl p-5 shadow border border-orange-100 flex items-center justify-between">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={settings.chocoQuizEnabled}
              onChange={(e) => handleSettingChange('chocoQuizEnabled', e.target.checked)}
        ...
        <div className="md:col-span-2 bg-orange-50 rounded-xl p-5 shadow border border-orange-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 space-grotesk">
            <div>
              ...
              <input
                type="date"
                value={settings.enrollmentPeriod.start}
                onChange={(e) => handleSettingChange('enrollmentPeriod.start', e.target.value)}
                className="p-3 w-full border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
            <div>
              ...
              <input
                type="date"
                value={settings.enrollmentPeriod.end}
                onChange={(e) => handleSettingChange('enrollmentPeriod.end', e.target.value)}
                className="p-3 w-full border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
            </div>
          </div>
      ...
      <div className="mt-8 text-center">
        <button
          ...
          className="px-8 py-3 bg-orange-600 hover:bg-orange-700 text-white font-bold rounded-lg shadow transition-colors duration-200 space-grotesk text-lg"
        >
...
const AdminDashboard = () => {
  ...
  const sidebarLinks = [
    ...
    { id: 'approvals', name: 'Duyệt GV', icon: '✅' }, // Keep existing
    { id: 'rewards', name: 'Kho Quà Tặng', icon: '🎁' }, // Keep existing
  ];
  ...
  const renderSection = () => {
    switch (activeSection) {
      ...
      case 'approvals':
        return <ApprovalManager />; // Assuming ApprovalManager and RewardManager are still relevant
      case 'rewards':
  ...
  // Fetch initial profile data if available (e.g., from localStorage or API)
  useEffect(() => {
    const storedRole = localStorage.getItem('userRole');
    const storedName = localStorage.getItem('userName'); // Assuming you might store user name
      ...
      <div className="w-72 bg-gradient-to-b from-gray-800 to-gray-900 text-white shadow-lg flex flex-col space-y-6 p-5">
        ...
        <div className="mt-auto">
          <button
            ...
            className="w-full p-3 bg-red-600 hover:bg-red-700 text-white rounded-lg shadow transition-colors duration-200 font-bold space-grotesk flex items-center justify-center"
          >
...
// --- Existing Components (Copied for completeness, assuming they are needed) ---

// Approval Management Component (from previous version)
const ApprovalManager = () => {
  ...
  const approveTeacher = (teacherId) => {
    console.log(`Phê duyệt giáo viên ID: ${teacherId}`);
    setPendingTeachers(pendingTeachers.filter(t => t.id !== teacherId));
    alert('Đã phê duyệt hồ sơ giáo viên!');
  };

  const rejectTeacher = (teacherId) => {
    console.log(`Từ chối giáo viên ID: ${teacherId}`);
    setPendingTeachers(pendingTeachers.filter(t => t.id !== teacherId));
    alert('Đã từ chối hồ sơ giáo viên.');
  };

  return (
    <AdminSectionCard title="Duyệt Hồ Sơ Giáo Viên" icon="✅">
      {pendingTeachers.length === 0 ? (
        <p className="text-center text-orange-600 text-lg space-grotesk">Không có hồ sơ giáo viên nào đang chờ xử lý.</p>
      ) : (
        <div className="space-y-4">
          {pendingTeachers.map(teacher => (
            <div key={teacher.id} className="bg-orange-50 rounded-xl p-4 shadow border border-orange-100 flex flex-col md:flex-row md:items-center justify-between space-grotesk">
              <div>
                <p className="font-bold text-orange-800">{teacher.name}</p>
                <p className="text-sm text-orange-600">Email: {teacher.email}</p>
                <p className="text-sm text-orange-500">Ngày đăng ký: {teacher.appliedDate}</p>
              </div>
              <div className="flex space-x-3 mt-3 md:mt-0">
                <button onClick={() => approveTeacher(teacher.id)} className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg shadow transition-colors duration-200 flex items-center space-x-1">
                  <span className="font-bold">✓</span> <span>Phê duyệt</span>
                </button>
                <button onClick={() => rejectTeacher(teacher.id)} className="px-4 py-2 bg-red-700 hover:bg-red-800 text-white rounded-lg shadow transition-colors duration-200 flex items-center space-x-1">
                  <span className="font-bold">✕</span> <span>Từ chối</span>
                </button>
              </div>
            </div>
          ))}
...
// Reward Manager Component (from previous version)
const RewardManager = () => {
  const [rewards, setRewards] = useState([
    { id: 1, name: 'Ly Cacao Nóng', cost: 50, imageUrl: '☕' },
    { id: 2, name: 'Sổ Tay Cacao', cost: 75, imageUrl: '✍️' },
    { id: 3, name: 'Voucher Học Tập', cost: 100, imageUrl: '📜' },
  ]);
  ...
  const addReward = () => {
    ...
    const newReward = { id: rewards.length + 1, name: newRewardName, cost: cost, imageUrl: newRewardImage || '🎁' };
    setRewards([...rewards, newReward]);
    setNewRewardName(''); setNewRewardCost(''); setNewRewardImage('');
  };
  ...
  return (
    <AdminSectionCard title="Kho Quà Tặng" icon="🎁">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        {rewards.map(reward => (
          <div key={reward.id} className="bg-orange-50 rounded-xl p-5 shadow border border-orange-100 flex items-center justify-between space-grotesk">
            <div className="flex items-center space-x-3">
              <span className="text-3xl">{reward.imageUrl}</span>
              <div>
                <p className="font-semibold text-orange-800">{reward.name}</p>
                <p className="text-sm text-orange-600">{reward.cost} Hạt Cacao</p>
              </div>
            </div>
            <button onClick={() => deleteReward(reward.id)} className="px-3 py-1 bg-red-500 hover:bg-red-600 text-white rounded-md text-sm transition-colors duration-200">
              Xóa
            </button>
          </div>
        ))}
      ...
      <div className="bg-orange-50 p-6 rounded-xl border border-orange-200 grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
        <div className="sm:col-span-1">
          <label className="block text-sm font-medium text-orange-700 mb-1 space-grotesk">Tên Quà Tặng</label>
          <input type="text" value={newRewardName} onChange={(e) => setNewRewardName(e.target.value)} placeholder="Ví dụ: Ly Cacao Sữa" className="w-full p-3 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 space-grotesk" />
        </div>
        <div className="sm:col-span-1">
          <label className="block text-sm font-medium text-orange-700 mb-1 space-grotesk">Số Hạt Cacao</label>
          <input type="number" value={newRewardCost} onChange={(e) => setNewRewardCost(e.target.value)} placeholder="100" className="w-full p-3 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 space-grotesk" />
        </div>
        <div className="sm:col-span-1">
          <label className="block text-sm font-medium text-orange-700 mb-1 space-grotesk">Icon (Emoji)</label>
          <input type="text" value={newRewardImage} onChange={(e) => setNewRewardImage(e.target.value)} placeholder="🎁" className="w-full p-3 border border-orange-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 space-grotesk" />
        </div>
        <div className="sm:col-span-3">
          <button onClick={addReward} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold py-3 px-4 rounded-lg transition-colors duration-200 shadow-md space-grotesk">
            Thêm Quà Tặng Vào Kho
          </button>
        </div>
      </div>
    </AdminSectionCard>
...
// System Overview Component (from previous version)
